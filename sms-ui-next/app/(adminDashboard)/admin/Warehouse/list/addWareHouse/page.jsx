...
const [states, setStates] = useState([]); // Filtered states
  ...
  const orgid = userData.orgid
    ...
    const validationSchema = Yup.object().shape({
      ...
      country: Yup.object()
    .shape({
        value: Yup.number().required("Country is required"), // string se number
        label: Yup.string().required(),
    })
    .nullable()
    ...
    const getCountryState = async () => {
        ...
        if (response.status == 200) {
            ...
            const transformedStates = response?.data?.dropdowns?.states?.map(state => ({
                value: state.id,
                label: state.name,
                countryId: state.countryId // Add countryId for filtering
            }));
            setAllStates(transformedStates);
            // setStates(transformedStates) // Initialize filtered states
            const roleId = response?.data?.dropdowns?.roles.find((item) => item?.name == 'Warehouse User');
        ...
        <div>
            <form className="max-w-7xl mx-auto p-6 rounded-md shadow-md" onSubmit={handleSubmit(onSubmit)}>
                <div className='flex justify-between items-center mb-6'>
                    <div className='text-orange-500 text-2xl bold'>Add Warehouse</div>
                </div>
                <div className="grid grid-cols-1 gap-4">
                    <div>
                        <div className="flex items-center space-x-4">
                            <label htmlFor="productName" className="w-1/4 font-medium dark:text-white text-gray-700">Warehouse Name *</label>
                            <Controller
                                name="stationName"
                                control={control}
                                render={({ field }) => (
                                    <input
                                        ...
                                        id="productName"
                                        {...field}
                                        className={`flex-grow p-2 border ${errors.stationName ? 'border-red-500': 'border-gray-300'}  rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500`}
                                    />
                                )}
                            />
                        </div>
                            ...
                            <Controller
                                ...
                                render={({ field }) => (
                                    <input
                                        ...
                                        id="email"
                                        {...field}
                                        className={`flex-grow p-2 border ${errors.email ? 'border-red-500': 'border-gray-300'} rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500`}
                                    />
                                )}
                            />
                        </div>
                            ...
                            <Controller
                                ...
                                render={({ field }) => (
                                    <input
                                        ...
                                        id="mobileno"
                                        {...field}
                                        className={`flex-grow p-2 border ${errors.mobileno ? 'border-red-500': 'border-gray-300'} border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500`}
                                    />
                                )}
                            />
                        </div>
...
<div>
  <div className="flex items-center space-x-4">
    <label htmlFor="country" className="w-1/4 font-medium dark:text-white text-gray-700">
      Country
    </label>
    <Controller
      name="country"
          ...
          onChange={(selectedOption) => {
            ...
            if (selectedOption) {
              const filteredStates = allStates.filter(
                (s) => Number(s.countryId) === Number(selectedOption.value)
              );
...
<div>
  <div className="flex items-center space-x-4">
    <label htmlFor="state" className="w-1/4 font-medium dark:text-white text-gray-700">
      State
    </label>
    <Controller
      name="state"
      ...
      render={({ field }) => (
        <Select
          ...
          id="state"
          options={states}
          isDisabled={states.length === 0} // disable until country is selected
          onChange={(selectedOption) => {
            field.onChange(selectedOption);
            clearErrors("state");
          }}
          className={`flex-grow text-black ${errors.state ? "border border-red-500" : ""}`}
        />
      )}
    />
  </div>
