...
const fetchDataGet = async () => {
  try {
    ...
    // Make sure this variable is defined in your state
    if (typeof setExcelFileUrl === 'function') {
      setExcelFileUrl(response.data.excelImportApi);
    }
    
    // Create the new filterData object
    const newFilterData = {
    ...
    console.log("New filter data:", newFilterData);
    
    setLoading(false);
  } catch (error) {
    console.error("Error fetching data:", error, error.response || 'No response data');
    setError(error);
    setLoading(false);
  }
};
...
useEffect(() => {
  ...
  const handleAcceptProduct = async (productId) => {
    try {
      setLoading(true);
      const url = `v2/Product/AcceptSKU?productid=${productId}&skustatus=88`;
      const response = await CallFor(url, "POST", null, "Auth");
      
      if (response.status === 200) {
        // If successful, refresh the data
        fetchData();
      } else {
        setError("Failed to accept product");
      }
      setLoading(false);
    } catch (error) {
    ...
    <div className="container mx-auto">
      ...
      <div className="mb-4 grid grid-cols-1 lg:grid-cols-2 gap-x-10">
          ...
          <Select
            ...
            value={
              filterModel.manufacturer
                ? {
                    value: filterModel.manufacturer,
                    label: filterData.manufacturer.find(
                      (m) => m.id === filterModel.manufacturer
                    )?.name,
                  }
                : null
            }
        ...
        <div className="flex items-center mb-2">
          ...
          <Select
            options={filterData.skustatus.map((s) => ({
              value: s.mvid,
              label: s.mastervalue1,
            }))}
            value={
              filterModel.skustatus
                ? {
                    value: filterModel.skustatus,
                    label: filterData.skustatus.find(
                      (s) => s.mvid === filterModel.skustatus
                    )?.mastervalue1,
                  }
                : null
            }
            onChange={(selected) =>
              handleInputChange("skustatus", selected ? selected.value : null)
            }
            className="w-3/4 text-black"
            isClearable
          />
      ...
      <div className="flex justify-between items-center pb-3">
        <div className="text-2xl text-orange-400">Requested Product</div>
        {/* <div>
          <Dialog>
      ...
      <table className="min-w-full text-left">
        ...
        <thead>
          <tr>
            ...
            <th className="px-2 py-2 cursor-pointer">STATUS</th>
          ...
          {data.map((item) => (
            <tr key={item.id}>
              ...
              <td className="px-2 py-2">
                <div className="w-24 h-24 bg-white overflow-hidden rounded-md shadow-md">
                  <img
              ...
              <td className="px-2 py-2 text-center">
                {item?.provariants[0]?.pvstatus == 87 ? (
                  <p className="border border-blue-500 inline-block text-blue-500 px-2 rounded">
                    All
                  </p>
                ) : item?.provariants[0]?.pvstatus == 88 ? (
                  <p className="border border-green-500 inline-block text-green-500 px-2 rounded">
                    Added
                  </p>
                ) : item?.provariants[0]?.pvstatus == 89 ? (
                  <p className="border border-red-500 inline-block text-red-500 px-2  rounded">
                    Rejected
                  </p>
                ) : (
                  <p className="border border-yellow-500 inline-block text-yellow-500 px-2  rounded">
                    NotAdded
                  </p>
                )}
              </td>
              <td className="px-2 py-2">
                {item?.provariants[0]?.pvstatus == 90 ? (
                     ...
                     <TooltipProvider>
                          <Tooltip>
                            <TooltipTrigger asChild>
                             <Link
                      href={`/admin/Catalogue/Requests/viewrequest/${item.proid}`}
                    >
                    ...
                    {/* Accept button with tick icon */}
                     ...
                     <TooltipProvider>
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button 
                      className="p-0 mr-2 bg-transparent hover:bg-transparent text-green-500"
                      onClick={() => handleAcceptProduct(item.proid)}
                  ...
                  <div>
                    ...
                    {/* Only show Accept button if status is not already "Added" (88) */}
                    {item?.provariants[0]?.pvstatus !== 88 && (
