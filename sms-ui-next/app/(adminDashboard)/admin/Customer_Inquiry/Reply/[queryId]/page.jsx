...
function ViewInquiry() {
  ...
  const getInquiryDetail = async () => {
    const response = await CallFor(
      `v2/Greviance/GetGrevianceDetailsList?Gid=${param.queryId}`,
      "GET",
      null,
      "Auth"
    );
  ...
  const handleReplySubmit = async (e) => {
    ...
    if (replyMessage !== "") {
      ...
      const response = await CallFor(
        `v2/Greviance/SaveGrevianceReplay`,
        "POST",
        JSON.stringify(data),
        "Auth"
      );
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto bg-white rounded-xl shadow-md space-y-4">
      ...
      <div className="flex justify-between items-center">
        ...
        <span
          className={`text-sm font-medium ${
            data.priority === "0" ? "text-red-600" : "text-green-600"
          }`}
        >
      ...
      <div className="mt-4">
        ...
        <p
          className={`text-sm font-semibold ${
            data.iscompleted ? "text-green-600" : "text-blue-600"
          }`}
        >
      ...
      <div className="mt-4">
        ...
        <button
          className={`mx-2 bg-red-500 px-4 py-2 rounded`}
          onClick={() => {
            router.push("/admin/Customer_Inquiry");
          }}
        >
          Back
        </button>
