...
const CustomerInquiry = () => {
  ...
  const getCustomerInquiryList = async () => {
    const response = await CallFor(`v2/Greviance/GetGrevianceList?PageNumber=1&PageSize=2&Uoid=0`,
      'POST',
          ...
          {data.map((item) => (
            <tr key={item.id}>
              ...
              <td className="px-2 py-2">
                  {" "}
                  <Button color="warning" className="shadow-md" onClick={()=>router.push(`/admin/Customer_Inquiry/Reply/${item?.gid}`)}>
                    <Undo size={20} className="pr-1" />
                    Reply
                  </Button>{" "}
                  {" "}
                  <Button color="warning" className="shadow-md" onClick={()=>router.push(`/admin/Customer_Inquiry/query/${item?.gid}`)}>
