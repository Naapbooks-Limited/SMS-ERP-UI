...
const requisition = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await CallFor("v2/Orders/GetMaterialRequests/False", "post", body, "Auth");
  ...
  return (
    <div className="container mx-auto">
        ...
        <div className="flex items-center mb-2">
          ...
          max={
            searchParams.requestDate
              ? new Date(
                  new Date(searchParams.requestDate).setDate(
                    new Date(searchParams.requestDate).getDate() + 30
                  )
                )
                  .toISOString()
                  .split("T")[0]
      ...
      <div className="flex justify-between gap-1 pb-3">
        <div className="text-2xl text-orange-400">Quotation</div>
        <div className="">
        ...
        <tbody>
          {data.map((item) => (
            <tr key={item.id}>
              ...
              <td className="px-2 py-2">{item.sourceOrgName}</td>
              ...
              <td className="px-2 py-2">
                {item.status == 78 ? (
                  ...
                  <p className="border-2 border-gray-500 inline-block text-gray-500 px-2 rounded-full">QuoteSent</p>
                ) : item.status == 106 ? (
                      <p className="border-2 border-red-500 inline-block text-red-500 px-2 rounded-full">Quote Rejected</p>
                ) : item.status == 80 ? (
                  <p className="border-2 border-green-500 inline-block text-green-500 px-2 rounded-full">SO Received</p>
                      ) : item.status == 107 ? (
                ...
                <div>
                 {item.status == 78 ? (
                  ...
                  <div className="flex mb-2">
                    <Link href={`/warehouse/sales/quotation/viewquotation/${item.mrid}`}>
                      <Button
                        className="p-0 mr-2 bg-transparent hover:bg-transparent text-black dark:text-white text-sm"
                      >
                        <Eye size={15}></Eye>
                      </Button>
                    </Link>
                    <Link href={`/warehouse/sales/quotation/createquotation/${item.mrid}`}>
                      <Button className="p-0 bg-[#11375C] hover:bg-[#1f5081] text-white text-sm mr-2 px-1">
                        Create Quote
                      </Button>
                    </Link>
                    ...
                    <Button className="p-0 bg-transparent hover:bg-transparent text-black dark:text-white text-sm" onClick={() => handleDeleteUser(item.mrid)}>
                      <Trash size={20}></Trash>
                    </Button>
                  </div>
                  </>
                  ...
                  <div className="flex mb-2">
                    <Link href={`/warehouse/sales/quotation/Viewquatationaccept/${item.mrid}`}>
                      <Button
                        className="p-0 mr-2 bg-transparent hover:bg-transparent text-black dark:text-white text-sm"
                      >
                        <Eye size={15}></Eye>
