...
const EditProductForm = ({params}) => {
    ...
    const fetchProductData = async () => {
      try {
        ...
        setFormData(prevData => ({
          ...prevData,
          ...productData,
          pvsalesprice: productData.pvsalesprice || "",
          pvpurchaseprice: productData.pvpurchaseprice || "",
          pvoldprice: productData.pvoldprice || "",
          pvspecialprice: productData.pvspecialprice || "",
          pvspstartdate: productData.pvspstartdate || "",
          pvspenddate: productData.pvspenddate || "",
          availabledate: productData.availabledate || "",
          enddate: productData.enddate || "",
          openingstock: productData.openingstock || "",
          openingstockdate: productData.openingstockdate || "",
          closingstock: productData.closingstock || "",
          closingstockdate: productData.closingstockdate || "",
          markAsNewStartDateTimeUtc: productData.markAsNewStartDateTimeUtc || "",
          markAsNewEndDateTimeUtc: productData.markAsNewEndDateTimeUtc || "",
        }));
        ...
        // Set attributes
        const attributes = productData.pvamappings.map(mapping => ({
        ...
        if (productData.pvdefaultimgUmUrl) {
          setImagePreview(productData.pvdefaultimgUmUrl);
        }
      } catch (error) {
  ...
  const handleImageUpload = async () => {
    ...
    try {
      ...
      setFormData(prevData => {
        const updatedPvummappings = prevData.pvummappings.map(mapping => {
          if (mapping.proid === prevData.proid) {
            return { ...mapping, productimgid: imageId };
          }
          return mapping;
        });

        // If no existing mapping was found, add a new one
        if (!updatedPvummappings.some(mapping => mapping.productimgid === imageId)) {
          updatedPvummappings.push({
            pvumid: 0,
        ...
        return {
          ...prevData,
          pvummappings: updatedPvummappings
        };
      });
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await CallFor("v2/Product/UpdateProductVariant", "post", JSON.stringify(formData), "Auth");
      if (!response.ok) {
        ...
        <div className="dark:text-white p-6 rounded-lg shadow-md space-y-4">
          ...
          <div className="flex items-center space-x-4">
            ...
            <select
              ...
              onChange={handleChange}
              className="w-3/4 mt-1 block p-2 border border-gray-300 text-black rounded-md shadow-sm focus:outline-none focus:ring focus:border-blue-300"
            >
              <option>1</option>
            </select>
          </div>
