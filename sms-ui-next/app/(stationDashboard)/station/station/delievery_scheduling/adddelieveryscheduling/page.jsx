...
const containerClasses = 'flex items-center space-x-4 mt-4';
...
const CustomTimePicker = ({ value, onChange, className }) => {
    const [hours, setHours] = useState(() => {
        ...
        return hour ? parseInt(hour, 10) : 0; // Default to 0 if value is empty or invalid
    });

    const [minutes, setMinutes] = useState(() => {
        const [, minute] = value.split(':');
        return minute ? parseInt(minute, 10) : 0; // Default to 0 if value is empty or invalid
    });

    const [isAM, setIsAM] = useState(() => {
        const [hour] = value.split(':');
        return hour ? parseInt(hour, 10) < 12 : true; // Default to AM if value is empty or invalid
    });
...
const Card = () => {
    ...
    useEffect(() => {
        const fetchShippingMethods = async () => {
            ...
            if (response) {
                ...
                reToast.error("Error fetching shipping methods.");
            }
        };
    ...
    const formatTime = (time) => {
        return time;  // The time is already in the desired format
    };
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            const formattedTimeSlot = `${formatTime(timeSlot.start)}-${formatTime(timeSlot.end)}`;
            ...
            const response = await CallFor2('api-fe/DeliverySlotAdminAPI/Create', 'POST', apiBody, 'Auth');
            if (response) {
                reToast.success("Saved successfully!");
                router.push("/station/station/delievery_scheduling");
            } else {
                reToast.error("Error saving user.");
                console.error("Failed to create delivery slot:", response.statusText);
            }
    ...
    return (
        <div className="mx-auto p-6">
            <h1 className="text-2xl font-bold text-orange-400 mb-6">Add New Delivery Slot</h1>
            <form onSubmit={handleSubmit}>
                <div className="grid grid-cols-1">
                    ...
                    <div className="flex items-center space-x-4">
                        ...
                        {formErrors.end && <p className="text-red-500 pl-7 text-sm">{formErrors.end}</p>}
                    </div>
                    <div className={containerClasses}>
