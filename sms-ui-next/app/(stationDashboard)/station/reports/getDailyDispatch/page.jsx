...
const getDailyDispatch = () => {
    ...
    const getOrderForReport = async () => {
        const response = await CallFor(`v2/Report/DailyDispatch?UaId=${userData?.uaid}&From=${searchQuery?.FromDate}&Uaidto=${userData?.uaid}`, 'GET', null, 'Auth');
            ...
            <table className="min-w-full text-left">
                ...
                <thead>
                    <tr>
                        ...
                        <th className="px-4 py-2 cursor-pointer" onClick={() => handleSort('todaydate')}>
                            Date{sortConfig.key === 'todaydate' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                ...
                <tbody>
                    {data?.map((item, index) => (
                        <tr key={index} className="border-b">
                            ...
                            <td className="px-4 py-2">{formatDate(item?.todaydate)}</td>
                            <td className="px-4 py-2">{item?.['amd - CO - SP - SBR']?.toFixed(4)}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
...
export default getDailyDispatch;
