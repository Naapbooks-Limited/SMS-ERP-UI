...
const customerRetained = () => {
    ...
    const getCustomerReport = async () => {
        const response = await CallFor(`v2/Report/CustomerRetained?RType=1`, 'GET', null, 'Auth');
                ...
                <thead>
                    <tr>
                        ...
                        <th className="  px-4 py-2 cursor-pointer" onClick={() => handleSort('username')}>
                            Product Name {sortConfig.key === 'username' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className="  px-4 py-2 cursor-pointer" onClick={() => handleSort('email')}>
                            Open stock{sortConfig.key === 'email' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className=" px-4 py-2 cursor-pointer" onClick={() => handleSort('phone')}>
                            Receipts {sortConfig.key === 'phone' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className=" px-4 py-2 cursor-pointer" onClick={() => handleSort('phone')}>
                            Sales  {sortConfig.key === 'phone' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className=" px-4 py-2 cursor-pointer" onClick={() => handleSort('phone')}>
                            Wastage {sortConfig.key === 'phone' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className=" px-4 py-2 cursor-pointer" onClick={() => handleSort('phone')}>
                            Transferred {sortConfig.key === 'phone' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className=" px-4 py-2 cursor-pointer" onClick={() => handleSort('phone')}>
                            Closing {sortConfig.key === 'phone' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                ...
                <tbody>
                    {
                        ...
                        data?.map((item, index) => (
                            <tr key={index}>
                                ...
                                <td className="px-2 py-2">{item?.openingStock}</td>
                                <td className="px-2 py-2">{item?.receipts}</td>
                                <td className="px-2 py-2">{item?.sales}</td>
                                <td className="px-2 py-2">{item?.wastage}</td>
                                <td className="px-2 py-2">{item?.transferred}</td>
                                <td className="px-2 py-2">{item?.closing}</td>
                            </tr>
                        ))
                        // :
...
export default customerRetained;
