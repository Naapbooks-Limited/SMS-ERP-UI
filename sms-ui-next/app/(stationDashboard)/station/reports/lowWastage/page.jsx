...
const lowWastage = () => {
    ...
    const getOrderForReport = async () => {
        const response = await CallFor(`v2/Report/LowWastage?OrgId=${userData?.orgid}&UaId=${userData?.uaid}&From=${searchQuery?.FromDate}&To=${searchQuery?.Todate}&NoRecords=1`, 'GET', null, 'Auth');
    ...
    return (
        <div className="container mx-auto mb-3">
            <div className='flex '>
                <h1 className="text-[20px] pl-2 font-semibold mb-4 text-orange-400">Low Wastage Report</h1>
            </div>
                ...
                <thead>
                    <tr>
                        <th className="  px-4 py-2 cursor-pointer" onClick={() => handleSort('id')}>
                            Wastage Id{sortConfig.key === 'id' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>

                        <th className="  px-4 py-2 cursor-pointer" onClick={() => handleSort('username')}>
                            Wastage type{sortConfig.key === 'username' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className="  px-4 py-2 cursor-pointer" onClick={() => handleSort('email')}>
                            Wastage qty{sortConfig.key === 'email' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className=" px-4 py-2 cursor-pointer" onClick={() => handleSort('phone')}>
                            Wastage value {sortConfig.key === 'phone' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                    </tr>
                </thead>
                {/* Table data */}
                <tbody>
                    {
                        ...
                        data?.map((item, index) => (
                            <tr key={index}>
                                <td className="px-2 py-2">{item?.wastageid}</td>
                                <td className="px-2 py-2">{item?.wastagetype}</td>
                                <td className="px-2 py-2">{item?.wastageqty}</td>
                                <td className="px-2 py-2">{item?.wastagevalue}</td>
                            </tr>
                        ))
                        // :
