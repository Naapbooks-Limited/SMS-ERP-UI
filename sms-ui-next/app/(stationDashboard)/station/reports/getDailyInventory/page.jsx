...
const getDailyInventory = () => {
    ...
    const getOrderForReport = async () => {
        const response = await CallFor(`v2/Report/GetDailyInventory?From=${searchQuery?.FromDate}&To=${searchQuery?.Todate}&Uaid=${userData?.uaid}`, 'GET', null, 'Auth');
    ...
    return (
        <div className="container mx-auto mb-3">
            <div className='flex '>
                <h1 className="text-[20px] pl-2 font-semibold mb-4 text-orange-400">Lowest sales Report</h1>
            </div>
            ...
            <table className="min-w-full text-left">
                ...
                <thead>
                    <tr>
                        <th className="px-4 py-2 cursor-pointer" onClick={() => handleSort('proId')}>
                            Product ID{sortConfig.key === 'proId' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className="px-4 py-2 cursor-pointer" onClick={() => handleSort('proName')}>
                            Product Name{sortConfig.key === 'proName' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className="px-4 py-2 cursor-pointer" onClick={() => handleSort('uom')}>
                            UOM{sortConfig.key === 'uom' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className="px-4 py-2 cursor-pointer" onClick={() => handleSort('openingStock')}>
                            Opening Stock{sortConfig.key === 'openingStock' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className="px-4 py-2 cursor-pointer" onClick={() => handleSort('receipts')}>
                            Receipts{sortConfig.key === 'receipts' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className="px-4 py-2 cursor-pointer" onClick={() => handleSort('sales')}>
                            Sales{sortConfig.key === 'sales' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className="px-4 py-2 cursor-pointer" onClick={() => handleSort('wastage')}>
                            Wastage{sortConfig.key === 'wastage' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className="px-4 py-2 cursor-pointer" onClick={() => handleSort('transferred')}>
                            Transferred{sortConfig.key === 'transferred' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className="px-4 py-2 cursor-pointer" onClick={() => handleSort('closing')}>
                            Closing{sortConfig.key === 'closing' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                    </tr>
                </thead>
                {/* Table data */}
                <tbody>
                    {data?.map((item, index) => (
                        <tr key={index} className="border-b">
                            <td className="px-4 py-2">{item?.proId}</td>
                            <td className="px-4 py-2">{item?.proName}</td>
                            <td className="px-4 py-2">{item?.uom}</td>
                            <td className="px-4 py-2">{parseFloat(item?.openingStock).toFixed(4)}</td>
                            <td className="px-4 py-2">{parseFloat(item?.receipts).toFixed(4)}</td>
                            <td className="px-4 py-2">{parseFloat(item?.sales || 0).toFixed(4)}</td>
                            <td className="px-4 py-2">{parseFloat(item?.wastage || 0).toFixed(4)}</td>
                            <td className="px-4 py-2">{parseFloat(item?.transferred).toFixed(4)}</td>
                            <td className="px-4 py-2">{parseFloat(item?.closing).toFixed(4)}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
...
export default getDailyInventory;
