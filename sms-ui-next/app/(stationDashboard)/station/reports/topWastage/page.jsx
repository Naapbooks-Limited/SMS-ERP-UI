...
const topWastage = () => {
    ...
    const getOrderForReport = async () => {
        const response = await CallFor(`v2/Report/GetWastage?OrgId=${userData?.orgid}&UaId=${userData?.uaid}&Config=2&From=${searchQuery?.FromDate}&To=${searchQuery?.Todate}`, 'GET', null, 'Auth');
    ...
    return (
        <div className="container mx-auto mb-3">
            <div className='flex '>
                <h1 className="text-[20px] pl-2 font-semibold mb-4 text-orange-400">Top Wastage Report</h1>
            </div>
            ...
            <table className="min-w-full text-left">
                ...
                <thead>
                    <tr>
                        <th className="px-4 py-2 cursor-pointer" onClick={() => handleSort('wastageid')}>
                            Wastage ID{sortConfig.key === 'wastageid' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className="px-4 py-2 cursor-pointer" onClick={() => handleSort('wastageno')}>
                            Wastage No{sortConfig.key === 'wastageno' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className="px-4 py-2 cursor-pointer" onClick={() => handleSort('productname')}>
                            Product Name{sortConfig.key === 'productname' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className="px-4 py-2 cursor-pointer" onClick={() => handleSort('pvname')}>
                            Variant Name{sortConfig.key === 'pvname' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className="px-4 py-2 cursor-pointer" onClick={() => handleSort('wastageqty')}>
                            Quantity{sortConfig.key === 'wastageqty' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className="px-4 py-2 cursor-pointer" onClick={() => handleSort('wastagedate')}>
                            Date{sortConfig.key === 'wastagedate' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className="px-4 py-2 cursor-pointer" onClick={() => handleSort('wastagetype')}>
                            Type{sortConfig.key === 'wastagetype' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className="px-4 py-2 cursor-pointer" onClick={() => handleSort('remarks')}>
                            Remarks{sortConfig.key === 'remarks' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className="px-4 py-2 cursor-pointer" onClick={() => handleSort('creator')}>
                            Created By{sortConfig.key === 'creator' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                    </tr>
                </thead>
                {/* Table data */}
                <tbody>
                    {data?.map((item, index) => (
                        <tr key={index} className="border-b">
                            <td className="px-4 py-2">{item?.wastageid}</td>
                            <td className="px-4 py-2">{item?.wastageno}</td>
                            <td className="px-4 py-2">{item?.productname}</td>
                            <td className="px-4 py-2">{item?.pvname}</td>
                            <td className="px-4 py-2">{item?.wastageqty?.toFixed(4)}</td>
                            <td className="px-4 py-2">{formatDate(item?.wastagedate)}</td>
                            <td className="px-4 py-2">{item?.wastagetype}</td>
