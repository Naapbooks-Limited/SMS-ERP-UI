...
const lowestSellingDay = () => {
    ...
    const getOrderForReport = async () => {
        const response = await CallFor(`v2/Report/LowestSellingDay?OrgId=${userData?.orgid}&UaId=${userData?.uaid}&From=${searchQuery?.FromDate}&To=${searchQuery?.Todate}&NoRecords=1`, 'GET', null, 'Auth');
                ...
                <thead>
                    <tr>
                        <th className="  px-4 py-2 cursor-pointer" onClick={() => handleSort('id')}>
                            From {sortConfig.key === 'id' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        ...
                        <th className="  px-4 py-2 cursor-pointer" onClick={() => handleSort('email')}>
                            Total Amount{sortConfig.key === 'email' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className=" px-4 py-2 cursor-pointer" onClick={() => handleSort('phone')}>
                            Quantity {sortConfig.key === 'phone' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className=" px-4 py-2 cursor-pointer" onClick={() => handleSort('phone')}>
...
export default lowestSellingDay;
