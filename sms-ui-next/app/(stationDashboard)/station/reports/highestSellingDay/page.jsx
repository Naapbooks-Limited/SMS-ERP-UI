...
const highestSellingDay = () => {
    ...
    const getOrderForReport = async () => {
        const response = await CallFor(`v2/Report/HighestSellingDay?OrgId=${userData?.orgid}&UaId=${userData?.uaid}&From=${searchQuery?.FromDate}&To=${searchQuery?.Todate}&NoRecords=1`, 'GET', null, 'Auth');
    ...
    return (
        <div className="container mx-auto mb-3">
            <div className='flex '>
                <h1 className="text-[20px] pl-2 font-semibold mb-4 text-orange-400">Highest selling Report</h1>
            </div>
                ...
                <thead>
                    <tr>
                        ...
                        <th className="  px-4 py-2 cursor-pointer" onClick={() => handleSort('username')}>
                            Total Item{sortConfig.key === 'username' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className="  px-4 py-2 cursor-pointer" onClick={() => handleSort('email')}>
                            qty{sortConfig.key === 'email' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className=" px-4 py-2 cursor-pointer" onClick={() => handleSort('phone')}>
                            Privous Rank {sortConfig.key === 'phone' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className=" px-4 py-2 cursor-pointer" onClick={() => handleSort('phone')}>
                            Current Rank{sortConfig.key === 'phone' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className=" px-4 py-2 cursor-pointer" onClick={() => handleSort('phone')}>
                            Order Date{sortConfig.key === 'phone' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                    </tr>
                </thead>
                {/* Table data */}
                <tbody>
                    {
                        ...
                        data?.map((item, index) => (
                            <tr key={index}>
                                <td className="px-2 py-2">{item?.addridentifier}</td>
                                <td className="px-2 py-2">{item?.itemtotal}</td>
                                <td className="px-2 py-2">{item?.qty}</td>
                                <td className="px-2 py-2">{item?.privousrank}</td>
                                <td className="px-2 py-2">{item?.currentrank}</td>
...
export default highestSellingDay;
