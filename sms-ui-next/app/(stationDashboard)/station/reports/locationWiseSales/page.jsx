...
const locationWiseSales = () => {
    ...
    const getOrderForReport = async () => {
        const response = await CallFor(`v2/Report/LocationWiseSales?From=${searchQuery?.FromDate}&To=${searchQuery?.Todate}`, 'GET', null, 'Auth');
    ...
    return (
        <div className="container mx-auto mb-3">
            <div className='flex '>
                <h1 className="text-[20px] pl-2 font-semibold mb-4 text-orange-400">LocationWise Sales Report</h1>
            </div>
                ...
                <thead>
                    <tr>
                        <th className="  px-4 py-2 cursor-pointer" onClick={() => handleSort('id')}>
                            Today Qty{sortConfig.key === 'id' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>

                        <th className="  px-4 py-2 cursor-pointer" onClick={() => handleSort('username')}>
                            Today Amount {sortConfig.key === 'username' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className="  px-4 py-2 cursor-pointer" onClick={() => handleSort('email')}>
                            LastWeek Qty{sortConfig.key === 'email' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className=" px-4 py-2 cursor-pointer" onClick={() => handleSort('phone')}>
                            LastWeek Amount {sortConfig.key === 'phone' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className=" px-4 py-2 cursor-pointer" onClick={() => handleSort('phone')}>
                            lastMonth Qty   {sortConfig.key === 'phone' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className=" px-4 py-2 cursor-pointer" onClick={() => handleSort('phone')}>
                            LastMonth Amount {sortConfig.key === 'phone' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                    </tr>
                </thead>
                {/* Table data */}
                <tbody>
                    {
                        ...
                        data?.map((item, index) => (
                            <tr key={index}>
                                <td className="px-2 py-2">{item?.todayQty}</td>
                                <td className="px-2 py-2">{item?.todayAmount}</td>
                                <td className="px-2 py-2">{item?.lastWeekQty}</td>
                                <td className="px-2 py-2">{item?.lastWeekAmount}</td>
                                <td className="px-2 py-2">{item?.lastMonthQty}</td>
                                <td className="px-2 py-2">{item?.lastMonthAmount}</td>
