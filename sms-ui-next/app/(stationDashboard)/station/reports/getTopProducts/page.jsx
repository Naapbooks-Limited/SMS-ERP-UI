...
const getTopProducts = () => {
    ...
    const getOrderForReport = async () => {
        const response = await CallFor(`v2/Report/GetTopProduct?OrgId=${userData?.orgid}&From=${searchQuery?.FromDate}&To=${searchQuery?.Todate}&Orderby=0&NoRecords=5&sortby=false`, 'GET', null, 'Auth');
    ...
    return (
        <div className="container mx-auto mb-3">
            <div className='flex '>
                <h1 className="text-[20px] pl-2 font-semibold mb-4 text-orange-400"> Top Prduct Report</h1>
            </div>
            ...
            <table className="min-w-full text-left">
                ...
                <thead>
                    <tr>
                        <th className="px-4 py-2 cursor-pointer" onClick={() => handleSort('pvid')}>
                            Product ID{sortConfig.key === 'pvid' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className="px-4 py-2 cursor-pointer" onClick={() => handleSort('pvname')}>
                            Product Name{sortConfig.key === 'pvname' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className="px-4 py-2 cursor-pointer" onClick={() => handleSort('pvalue')}>
                            Value{sortConfig.key === 'pvalue' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className="px-4 py-2 cursor-pointer" onClick={() => handleSort('pqty')}>
                            Quantity{sortConfig.key === 'pqty' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                        <th className="px-4 py-2 cursor-pointer" onClick={() => handleSort('orderdeliveryFrom')}>
                            Delivery From{sortConfig.key === 'orderdeliveryFrom' ? (sortConfig.direction === 'asc' ? '▲' : '▼') : ''}
                        </th>
                    </tr>
                </thead>
                {/* Table data */}
                <tbody>
                    {data?.map((item, index) => (
                        <tr key={index} className="border-b">
                            <td className="px-4 py-2">{item?.pvid}</td>
                            <td className="px-4 py-2">{item?.pvname}</td>
                            <td className="px-4 py-2">{item?.pvalue?.toFixed(4)}</td>
                            <td className="px-4 py-2">{item?.pqty?.toFixed(4)}</td>
                            <td className="px-4 py-2">{item?.deliveryFrom || '-'}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
