...
import { Eye, Mail, Plus, Search, SearchIcon, Trash } from "lucide-react";
  ...
  return (
    <div className="container mx-auto">
      ...
      <table className="min-w-full text-left">
        <thead>
          <tr>
            <th className="px-2 py-2 cursor-pointer">#SR</th>
            <th
            ...
            >
              Req. No
              {sortColumn === "mrid" && (sortDirection === "asc" ? "▲" : "▼")}
            </th>
            <th
              className="px-2 py-2 cursor-pointer"
              onClick={() => handleSort("orderdeliverfromname")}
            >
              WAREHOUSE
              {sortColumn === "orderdeliverfromname" &&
                (sortDirection === "asc" ? "▲" : "▼")}
            </th>
            ...
            >
              ORDERED QUANTITY
              {sortColumn === "totalReceivedQty" &&
                (sortDirection === "asc" ? "▲" : "▼")}
            </th>
            <th
              className="px-2 py-2 cursor-pointer"
              onClick={() => handleSort("totalOrderQty")}
            >
              SENT ITEMS
              {sortColumn === "totalOrderQty" &&
                (sortDirection === "asc" ? "▲" : "▼")}
            </th>
            <th
        ...
        <tbody>
          {data.map((item) => (
            <tr key={item.id}>
              ...
              <td className="px-2 py-2">{item?.materialRequest?.targetOrgName}</td>
              <td className="px-2 py-2">{item?.totalReceivedQty}</td>
                  ...
                  <div className="flex mb-2">
                    ...
                    <TooltipProvider>
                          <Tooltip>
                           ...
                           <Link
                      href={`/station/Purchase/po/viewpocancle/${item.orderid}`}
                    >
                      <Button
                        color="transprent"
                        ...
                        <TooltipProvider>
                          <Tooltip>
                            <TooltipTrigger asChild>
                             <Button
                      color="transprent"
                      className="p-0 text-black  dark:text-white text-sm "
                      onClick={() => handleDeleteUser(item.orderid)}
                    >
                      <Trash size={20} />
                    </Button>
                            </TooltipTrigger>
                        ...
                        {/* <TooltipProvider>
                          <Tooltip>
                            <TooltipTrigger asChild>
                               <Button
                      color="transprent"
                      className="p-0 dark:text-white text-black  px-2"
                      onClick={() => handleOpenMailBox(item.email)}
                    >
                      <Mail size={20} />
                    </Button>
                            </TooltipTrigger>
