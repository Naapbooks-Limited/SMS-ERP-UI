...
function ViewQuotation({ params }) {
  ...
  const createQuote = async () => {
    ...
    try {
      ...
      if (response) {
        reToast.success("PO created successfully!");
        router.push('/station/Purchase/Requistion');
      } else {
  ...
  return (
    ...
    <div className="grid grid-cols-2 p-3">
      ...
      <div>
        ...
        <label className="text-orange-500">{data?.targetOrgName}</label>
      </div>
      <div>
        ...
        {data?.status == 78 ? (
                  ...
                  <p className="border-2 border-red-500 inline-block text-red-500 px-2 rounded-full">Not fulfilled</p>
                ) :data?.status == 80 ? (
  ...
  <div className="overflow-x-auto bg-white dark:bg-gray-900 rounded-md shadow">
    <table className="w-full text-sm text-center border-collapse">
      ...
      <tbody>
        {data?.materialrequestitems?.map((item, index) => (
          ...
          >
            ...
            <td className="p-3">{item.proName || "N/A"}</td>
            <td className="p-3 min-w-[200px] text-left">{getAttributes(item.mridetails)}</td>
            <td className="p-3">{item.itemqty}</td>
            <td className="p-3">{item.avialableitemqty}</td>
            <td className="p-3"> {item.price?.toFixed(2)}</td>
            <td className="p-3"> {(item.avialableitemqty * item.price).toFixed(2)}</td>
      ...
      <div className="flex justify-end mt-6">
        <Button onClick={() => handleDeleteUser(params.mrid)} className="text-white bg-blue-950 mr-3">Reject</Button>
      ...
      <DeleteDialog
        ...
        delUrl={`v2/Orders/RejectMaterialRequest?id=${selectedUserId}`}
      />
