...
import { Stationmenus } from "@/config/Stationmenus";
...
const PopoverSidebar = () => {
  ...
  const menus = Stationmenus?.sidebarNav?.classic || [];
  const [activeSubmenu, setActiveSubmenu] = useState(null);
  const [activeMultiMenu, setMultiMenu] = useState(null);
  
     console.log(menus, "menus popover");
  ...
  return (
    <div
      className={cn("fixed  top-0  border-r bg-[#11375C]  ", {
        "w-[248px]": !collapsed,
        "w-[72px]": collapsed,
        "m-6 bottom-0   bg-card rounded-md": layout === "semibox",
        "h-full    ": layout !== "semibox" ,
      })}
    >
      {sidebarBg !== "none" && (
        <div
      ...
      >
        <ul
          className={cn(" space-y-1 rounded", {
